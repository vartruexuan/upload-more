...
layui.define(['upload', 'layer', 'sortable'], function (exports) {
    ...
    var MOD_NAME = 'uploadMore';  // 绑定事件的模块名
    ...
    uploadMore.prototype.bindEvents = function () {
        ...
        // 蒙版功能按钮能力
        this.container.delegate('.uploadMore-operation-action', 'mouseenter', function (e) {
            $(this).css({
                'color': $(this).hasClass('uploadMore-operation-action-delete') ? 'red' : '#4444d9',
            });
        });
        this.container.delegate('.uploadMore-operation-action', 'mouseleave', function (e) {
            $(this).css({
                'color': 'white',
            });
        });
        ...
        // 文件预览
        this.container.delegate('.uploadMore-operation-action-preview', 'click', function (e) {
            layui.stope(e);
        ...
        // 编辑图片
        this.container.delegate('.uploadMore-operation-action-edit', 'click', function (e) {
            layui.stope(e);
        });
    ...
    uploadMore.prototype.checkSize = function (file) {
        ...
        if (size > 0 && !(device.ie && device.ie < 10)) {
            if (file.size > 1024 * size) {
                ...
                limitSize = sizeStr;
                if (limitSize) {
                    that.msg(typeof text['limit-size'] === 'function'
                        ? text['limit-size'](that.upload.config, limitSize)
                        : '文件大小不能超过 ' + limitSize);
                }
                return false;
            }
    ...
    uploadMore.prototype.getItemTpl = function (index) {
        ...
        if (that.options.sortable !== false) {
            // 插入拖拽排序按钮
            item.append('<span class="uploadMore-drag"><span' +
        ...
        if (that.options.operation && that.options.operation.length > 0) {
            ...
            // 预览按钮
            if ($.inArray('preview', that.options.operation) !== -1 && that.isAllowPreview(itemInfo.mimeType)) {
                operationBox.find('.uploadMore-operation-box').append(
            ...
            // 删除按钮
            if ($.inArray('delete', that.options.operation) !== -1) {
                operationBox.find('.uploadMore-operation-box').append(
        ...
        item.append('<div class="uploadMore-progress">' +
        ...
        // 加上错误信息展示
        item.append('<div class="uploadMore-message layui-hide">' +
    ...
    uploadMore.prototype.getFileTpl = function (index) {
        ...
        if (mime.indexOf('image/') !== -1) {
            tpl = $('<div class=" uploadMore-file uploadMore-img-preview">' +
                '                <img src="' + (itemInfo ? itemInfo.url : '') + '"' +
